...
export default {
	...
	setup(props) {
		...
		const getSvgUrl = (file: string) => new URL(`../../assets/svg/svg/${file}.svg`, import.meta.url).href;
		// 逻辑判断
		if (props.name?.startsWith('ele-')) return () => h('i', eleSetStyle, [props.name === 'ele-' ? '' : h(resolveComponent(props.name))]);
		else if (props.name?.startsWith('')) return () => h('img', { src: getSvgUrl(props.name), style: localsStyle });
